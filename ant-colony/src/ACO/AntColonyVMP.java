...
//            break;
        ...
        if (usedAlgo == MigrationAlgo.ACO && vmAllocationPolicy instanceof VMAllocationACO)
    ...
    private double printHostCpuUsageAndPowerConsumption(final Host host) {
        ...
        System.out
                .println("-------------------------------------------------------------------------------------------");
        SortedMap<Double, Double> utilizationHistory = host.();
        // The total power the Host consumed in the period (in Watt-Sec)
        double totalHostPowerConsumptionWattSec = 0;
        int index = 0;
        for (Entry<Double, Double> entry : utilizationHistory.entrySet()) {
